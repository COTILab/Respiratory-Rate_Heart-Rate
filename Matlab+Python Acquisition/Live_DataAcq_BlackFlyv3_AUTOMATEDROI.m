...
Refresh_ROI_Frames=6; % Every X number of frames , when to update ROIs
...
if face_det_flag==1 % use facedetector
    % transpose images first -- FACE HAS TO BE UPRIGHT
    
    
    
    [Cascade_outputs]=py.CameraFunctions.set_predictor_cascade;
    %image_8bit=uint8(image_struct_first.images(:,:,100)/256);
    %% Find a default BBox first if there is an issue, we will overwrite any "Empty" BBox with this value
    tic
    for i=1:size(image_struct_first.images,3)
        image_8bit=uint8(image_struct_first.images(:,:,i)/256); % correcting scaling of image to 8 bit
        tic
        [def_BBox,def_landmarks]=get_ROI_Face_mat(image_8bit,sf,mn,Cascade_outputs);
        def_BBox_chest=get_ROI_Chest(def_BBox,def_landmarks,image_struct_first.dim1,image_struct_first.dim2);
        toc
        if size(def_BBox,1)==1
            disp("Default Face and Chest Coordinates acquired!");i
            break;
        end
        tic
        step(faceDetector,image_8bit);
        toc
        
    end
    %% Get BBoxes for our images
    ctr_ROI=1;
    temp_BBox=def_BBox;
    temp_BBox_Chest=def_BBox_chest;
    for i=1:size(image_struct_first.images,3)
        BBox_Face(i,:)=temp_BBox;
        BBox_Chest(i,:)=temp_BBox_Chest;
        if ctr_ROI>Refresh_ROI_Frames % if we want new ROIs
            image_8bit=uint8(image_struct_first.images(:,:,i)/256);
            [New_Face,New_Landmarks]=get_ROI_Face_mat(image_8bit,sf,mn,Cascade_outputs);
            New_Chest=get_ROI_Chest(New_Face,New_Landmarks,image_struct_first.dim1,image_struct_first.dim2);
            if size(New_Face,1)==1 % Found face
                temp_BBox=New_Face;
                temp_BBox_Chest=New_Chest;
                BBox_Face(i,:)=temp_BBox;
                BBox_Chest(i,:)=temp_BBox_Chest;
                ctr_ROI=1; % reset ctr_ROI once we assign a new ROI
            end
        ...
        %tic
        %[BBox,landmarks]=get_ROI_Face_mat(image_8bit,sf,mn,Cascade_outputs);
        %BBox_chest=get_ROI_Chest(BBox,landmarks,image_struct_first.dim1,image_struct_first.dim2);
        %toc
        % read dlib key
    ...
    clear dlib_key image_8bit
     [Avg_ROI,Avg_ROI_vec,new_time_stamps]=CroptoROI_FrameRange_MULTIPLEROIS_FACECHESTOPENCV(image_struct_first,BBox,init_tc_frame_count);
end
