...
image_struct_first=BlackFly_TakeImgv2(vid,75,cam_init_flag); % BLACKFLY
...
if face_det_flag==1 % use facedetector
    faceDetector = vision.CascadeObjectDetector('ScaleFactor',1.04); % initializing
    img_ctr=1;
    for i=1:size(image_struct_first.images,3)
       ...
       orig=temp; %
        ...
        if img_ctr==1
            BBox_bf=step(faceDetector,orig);
            points = detectMinEigenFeatures(orig, 'ROI',BBox_bf);  % Landmarks to follow
            ...
            initialize(pointTracker, points, orig);
        ...
        if img_ctr==0  % not the first frame
           [points, isFound] = step(pointTracker, orig); 
...
figure
for i=1:size(image_struct_first.images,3)
    imagesc(image_struct_first.images(:,:,i));colorbar
    pause(0.01)
    title(i)
