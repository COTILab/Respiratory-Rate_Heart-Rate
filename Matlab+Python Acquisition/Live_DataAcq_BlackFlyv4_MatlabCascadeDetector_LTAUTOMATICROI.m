...
% for open CV; Sf= Scaling factor.. lower = slower + more robust Face
% recognition. ,mn= minimum neighbors. larger = less false positives but
% higher chance of missing face
...
disp("NO STRUCTURE SENSOR SOURCE")
end
...
if face_det_flag==1 % use facedetector
    %% FACE HAS TO BE UPRIGHT FOR DETECTORS TO WORK
    %% Use Slower Cascade Detector on First Frame no matter what -- Gets landmarks for face
%     bit_8_img=uint8(image_struct_first.images/256); % 8 bit is faster to operate on
%     DetectedFaceStruct{1}=FaceDetectionRefresh(bit_8_img(:,:,1),faceDetector_IR,pointTracker_Face);
%     % Chest detector currently is a simple shift based on face ROI
%     DetectedChestStruct{1}=ChestDetectionRefresh(DetectedFaceStruct{1}.newBBox, bit_8_img(:,:,1),pointTracker_Chest,0);
%     
%     
%     
%     %[Cascade_outputs]=py.CameraFunctions.set_predictor_cascade;
...
%     
%     %% Sorting BBox Array into num_frames x num ROI cell array
    
    [DetectedFaceStruct,DetectedChestStruct,detail_struct]=GetFaceandChestROI(image_struct_first,faceDetector_IR,pointTracker_Face,pointTracker_Chest,KLT_conf,Refresh_ROI_Frames);
    
    for i=1:length(DetectedFaceStruct)
       BBox{i,1}= round(DetectedFaceStruct{i}.newBBox);
       BBox{i,2}= round(DetectedChestStruct{i}.Chest_BBox);
    end
    
    %% Done getting ROIs-- Now we crop...
    
  

clear dlib_key image_8bit

disp(" NO ABILITY TO REMOVE DUMMY FRAMES RIGHT NOW")
[Avg_ROI,Avg_ROI_vec,new_time_stamps,LastImage]=CroptoROI_FrameRange_MULTIPLEROIS_MatlabKLTCascade(image_struct_first,BBox,ROI_Mask_Arr);
% Visualizing just for testing
%
% figure;
% for i=1:size(Avg_ROI,1)
%     subplot(2,1,1)
%     imagesc(Avg_ROI{i,1});colorbar;title(i)
%     subplot(2,1,2);
%     imagesc(Avg_ROI{i,2});colorbar;title(i);
%     pause(0.01)
%     drawnow
% end
...
%% adding necessary trackers
Refresh_struct.faceDetector_IR=faceDetector_IR; % Cascade Detector
Refresh_struct.pointTracker_Face=DetectedFaceStruct{end}.pointTracker; % Point Tracker for face
Refresh_struct.pointTracker_Chest=pointTracker_Chest;
Refresh_struct.ROI_Mask_Arr=ROI_Mask_Arr;
Refresh_struct.KLT_conf=KLT_conf;
Refresh_struct.Refresh_ROI_Frames=Refresh_ROI_Frames;;
...
output_struct=Blackfly_HRBuffer_MULTIPLEROIS_CascadeKLTMatlab(Refresh_struct);
stop(vid)
