...
skinDetection.method                = 2;        % [0 1 2 3 4];  CHANGE THIS TO CHANGE METHOD RAHUL
disp(strcat("Skin detection Method chosen=",num2str(skinDetection.method ))); % added by RR
...
%4= Statistical masking: WRITTEN BY RAHUL-- Less sophisticated but should
%be far faster
...
%% select face based on template
    ...
    if faceDetection.nFacesDetected > 0 & boxInsideImage & boxBigEnough % detect skin every X frames, only if face is actually detected
        ...
        skinDetection.f = f;
        %rahul=tic 
        %debug_skinDetection=skinDetection; %% RAHUL ADDED
        ...
        elseif skinDetection.method == 2 % template based on k-clustering of colors
            ...
            %% skinDetection is original inp struct
            %% faceIm is double original face input --> Mask to uint8 for imshow to work
            %% skimImB_3 is logical array (x by y by 3 for 3 channels).  faceIm(~skinImB_3) = 0 masks our image as required
        ...
        elseif skinDetection.method == 4 % template based on statistical criterion- WRITTEN BY RAHUL
            [skinDetection,faceIm,skinImB_3] = selectFaceTemplateStatistics(skinDetection, Im);
