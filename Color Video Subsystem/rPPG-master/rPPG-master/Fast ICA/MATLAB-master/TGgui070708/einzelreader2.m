...
function einzelreader2(handles, h)
    ...
    iprod_thresh  = str2double(get(handles.iprod_thresh_edit,'String'))*pix_to_pho; %minimum threshold for a "bright object"    
...
%             infile = [file_info.path,'\',file_info.part_name,index,file_info.ext];
        ...
        drawnow;
        waitbarxmod(compl,w,'Executing "einzelreader2.m" ...'); %Rename the waitbar
        ...
        while(pix_val >= iprod_thresh) %continue until reach minimum threshold
            ...
            for i=rbox+1:y_size-rbox-1
                for j=rbox+1:x_size-rbox-1
                    if(high_pixel_mask(i,j)==0 && iprod_copy(i,j)>pix_val)
            ...
            beta=[xguess,yguess,50,psf_w0/q];
            [betafit,resid,J,COVB,mse] = nlinfit(xymerge,zmerge,@gaussian_merge2,beta);
            ci = nlparci(betafit,resid,'covar',COVB); %calculate error estimates on parameters
            ci_err=(ci(:,2)-ci(:,1))/2;

            zfitmerge=gaussian_merge2(betafit,xymerge);

            for i=1:wbox
                for j=1:wbox
                    k=(i-1)*wbox+j;
                    zfit(i,j)=zfitmerge(k);
                end
            end
            ...
            if(failed==0) %assign if fit criteria is satisfied
                ...
                pix_val_all(total_molecules)=pix_val;
        ...
        if preview  %If preview active, update images on screen
            ...
            %If preview active, compute wf & merge on first, last, and nth frames (always)
            if fileloop == n_end
...
%----------------
    if (file_info.part_name(length(file_info.part_name)) == '_') %Avoid saving with repeated seperators
        out_file = [outpath,file_info.part_name,num2str(n_start),'-',num2str(n_end),'_t',num2str(iprod_thresh),'.mat'];
    else
        out_file = [outpath,file_info.part_name,'_',num2str(n_start),'-',num2str(n_end),'_t',num2str(iprod_thresh),'.mat'];
    end
